...
public class GameState extends BaseState {
	...
	public GameState(String name, StateManager stateManager, InputHandler input, int width, int height) {
		...
		for (int i = 0; i < itemCount; i++) {
			...
			if (rand.nextInt(100) < 99) {
				...
				scenery.add(new SceneryEntity(Sprite.getSpriteFromSheet(
						...
						.nextInt(256), rand.nextFloat()+0.1f, rand.nextFloat()+1.0f));
			}
		}
	}
	public void sortRenderDepth(){
		for (int i = 1; i<scenery.size()-1; i++){
			...
			int holePos = i;
			while (holePos > 0 && se.renderDepth > scenery.get(holePos-1).renderDepth){
